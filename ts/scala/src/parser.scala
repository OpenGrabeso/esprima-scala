...
trait Context() {
  ...
  var await: Boolean = _
  var firstCoverInitializedNameError: RawToken | Null = _
...
class Parser(code: String, options: Any = new {}, var delegate: (AnyRef, AnyRef) => Any) {
  ...
  def unexpectedTokenError(token: RawToken | Null, message: String): Error = {
  ...
  def throwUnexpectedToken(token: RawToken | Null, message: String): never = {
    throw this.unexpectedTokenError(token, message)
  }
  
  def tolerateUnexpectedToken(token: RawToken | Null, message: String) = {
  ...
  def isolateCoverGrammar(parseFunction: () => AnyRef) = {
  ...
  def inheritCoverGrammar(parseFunction: () => AnyRef) = {
    val previousIsBindingElement = this.context.isBindingElement
    val previousIsAssignmentTarget = this.context.isAssignmentTarget
    val previousFirstCoverInitializedNameError = this.context.firstCoverInitializedNameError
    this.context.isBindingElement = true
      ...
      case Token.Punctuator =>
        this.lookahead.value match {
          ...
          case "/" | "/=" =>
            ...
            raw = this.getTokenRaw(token)
            expr = this.finalize(node, new Node.RegexLiteral(token.regex.asInstanceOf[RegExp], raw, token.pattern, token.flags))
          case _ =>
  ...
  def isPropertyKey(key: Node.PropertyKey | Null, value: String) = {
  ...
  def parseObjectProperty(hasProto: AnyRef): Node.Property = {
    ...
    val token = this.lookahead
    var kind: String = _
    var key: Node.PropertyKey | Null = null
    var value: Node.PropertyValue | Null = null
    var computed = false
    var method = false
    var shorthand = false
    var isAsync = false
  ...
  def checkPatternParam(options: AnyRef, param: ArrowParameterPlaceHolder) = {
    param.`type` match {
  ...
  def parseStatementListItem(): Node.StatementListItem = {
    var statement: Node.StatementListItem = _
    this.context.isAssignmentTarget = true
    this.context.isBindingElement = true
  ...
  def parseLexicalBinding(kind: String, options: AnyRef): Node.VariableDeclarator = {
    ...
    var init: Node.Expression | Null = null
    if (kind == "const") {
  ...
  def parsePropertyPattern(params: Array[Any], kind: String): Node.Property = {
    ...
    var shorthand = false
    val method = false
    var key: Node.PropertyKey | Null = _
  ...
  def parsePattern(params: Array[Any], kind: String): Node.BindingIdentifier | Node.BindingPattern = {
    var pattern = _
    if (this.`match`("[")) {
  ...
  def parsePatternWithDefault(params: Array[Any], kind: String): Node.AssignmentPattern | Node.BindingIdentifier | Node.BindingPattern = {
    val startToken = this.lookahead
  ...
  def parseIfStatement(): Node.IfStatement = {
    ...
    var alternate: Node.Statement | Null = null
  ...
  def parseForStatement(): Node.ForStatement | Node.ForInStatement | Node.ForOfStatement = {
    var init = null
    var test: Node.Expression | Null = null
    var update: Node.Expression | Null = null
    var forIn = true
    var left: AnyRef = _
    var right: AnyRef = _
    val node = this.createNode()
  ...
  def parseContinueStatement(): Node.ContinueStatement = {
    ...
    var label: Node.Identifier | Null = null
  ...
  def validateParam(options: AnyRef, param: ArrowParameterPlaceHolder, name: String) = {
    val key = "$" + name
    if (this.context.strict) {
  ...
  def parseFormalParameters(firstRestricted: RawToken | Null) = {
    object options {
      var simple = true
      var params = Array()
  ...
  def parseFunctionDeclaration(identifierIsOptional: Boolean): Node.AsyncFunctionDeclaration | Node.FunctionDeclaration = {
    ...
    var id: Node.Identifier | Null = null
    var firstRestricted: RawToken | Null = null
  ...
  def parseFunctionExpression(): Node.AsyncFunctionExpression | Node.FunctionExpression = {
    ...
    var message: String = _
    var id: Node.Identifier | Null = null
    var firstRestricted: RawToken | Null = _
    val previousAllowAwait = this.context.await
    val previousAllowYield = this.context.allowYield
    this.context.await = isAsync
    this.context.allowYield = !isGenerator
  ...
  def parseYieldExpression(): Node.YieldExpression = {
    ...
    var argument: Node.Expression | Null = null
    var delegate = false
    if (!this.hasLineTerminator) {
      val previousAllowYield = this.context.allowYield
      this.context.allowYield = false
  ...
  def parseClassElement(hasConstructor: AnyRef): Node.Property = {
    ...
    var kind: String = ""
    var key: Node.PropertyKey | Null = null
    var value: Node.FunctionExpression | Null = null
    var computed = false
    var method = false
    var isStatic = false
    var isAsync = false
  ...
  def parseClassDeclaration(identifierIsOptional: Boolean): Node.ClassDeclaration = {
    ...
    var superClass: Node.Identifier | Null = null
  ...
  def parseExportDeclaration(): Node.ExportDeclaration = {
    ...
    if (this.matchKeyword("default")) {
      ...
      // export var f = 1;
      var declaration: Node.StatementListItem = _
      this.lookahead.value match {
        case "let" | "const" =>
      ...
      var source: Node.Literal | Null = null
